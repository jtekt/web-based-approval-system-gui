...
export default {
    ...
    methods: {
        get_templates() {
            ...
            this.axios
                ...
                .then(({ data }) => {
                    ...
                    if (template)
                        this.selected_form =
                            this.application_form_templates.find(
        ...
        submit() {
            ...
            const recipients_ids = this.recipients.map(
                (recipient) => recipient._id || recipient.properties._id
            )
            const group_ids = this.groups.map(
                (group) => group._id || group.properties._id
            )
        ...
        add_to_recipients(new_recipient) {
            const existing_recipient = this.recipients.find(
                (recipient) =>
                    ...
                    this.get_id_of_item(new_recipient)
            )
            ...
            this.axios
                ...
                .then(({ data }) => {
                    ...
                    this.recipients = recipients.sort(
                        (a, b) =>
                            a.submission.flow_index - b.submission.flow_index
                    )
                })
                .catch((error) => {
                    console.error(error)
        ...
        add_group(group_to_add) {
            const group_exists = this.groups.some(
                ({ _id }) => _id === group_to_add._id
            )
